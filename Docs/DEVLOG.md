...
Added visible damage feedback to enemy group count labels. When player fire reduces the remaining enemy unit count, the count label now pulses red with a small safe lift/scale animation, then settles back to its clamped resting pose. Enemy groups now also bind their `Damageable` event path during configuration so pooled or validation-spawned groups reliably own count updates. The existing production validation now damages a spawned enemy by one inferred unit of health, confirms the label stays on remaining-unit count, and verifies a red damage pulse is applied.

Result:

`git diff --check` passed. Unity production validation passed with log `/tmp/armyrush_unity_enemy_count_pulse_validate.log`, including the new spawned-enemy damage-pulse guard. Targeted log scanning found no `error CS`, `Exception`, `NullReference`, `MissingReference`, `Fatal`, or validation-failure signatures after the final rerun.

Follow Up:

Physical iPhone QA should retest early enemies and confirm enemy numbers visibly tick down as progress, making damage output feel clearer without changing combat balance.
...
Fixed a physical-device combat readability regression where enemy group labels could be recalculated as remaining units by `EnemyGroup` and then overwritten by `Damageable` as raw HP after damage. Enemy group count labels now stay owned by `EnemyGroup`, reapply `WorldTextGuard` clamps on refresh, and production validation damages a spawned enemy group to prove the label remains a unit count instead of flipping to raw health.

Result:

`git diff --check` passed. Unity production validation passed with log `/tmp/armyrush_unity_enemy_count_label_validate.log`, and targeted log scanning found no `error CS`, `Exception`, `NullReference`, `MissingReference`, `Fatal`, or validation-failure signatures.

Follow Up:

Physical iPhone QA should retest the first enemy and early groups to confirm the number above enemies reads as remaining soldiers, not oversized or intimidating HP values.
...
Added a save-backed early retry power assist for physical iPhone first-session retention. Failed Levels 1-5 now record a bounded failure streak in save data; retrying the same level starts with a visible `RETRY POWER` combat boost that uses the existing pooled floating text and run-boost HUD paths, then the streak clears when that level is completed. The tuning asset and project builder own the retry boost level limit, duration, damage multiplier, and fire-rate multiplier, and production validation now guards those ranges against becoming a permanent progression replacement.

Result:

`git diff --check` passed. Unity production validation passed with log `/tmp/armyrush_unity_retry_power_validate.log`, and targeted log scanning found no `error CS`, `Exception`, `NullReference`, `MissingReference`, `Fatal`, or validation-failure signatures.

Follow Up:

Physical iPhone QA should intentionally lose one early run, retry the same level, and confirm `RETRY POWER` makes the next opener feel clearly stronger while the failure streak clears after victory.
...
Physical iPhone Onboarding Overdrive Tuning
...
Strengthened the bounded first-session combat assist for the physical iPhone onboarding ramp. Levels 1-3 now start with a clearer 24-second `POWER START` window using 35% damage and fire-rate boosts, 28m targeting, 48-speed projectiles, denser opening and post-gate volley visuals, stronger immediate hit confirmation, and a slightly stronger close-range urgency assist. The active boost HUD now tracks each boost's own duration so the longer onboarding countdown fill reflects `POWER START` correctly instead of using the shorter run-boost gate duration.

Result:

`git diff --check` passed. Unity production validation passed with log `/tmp/armyrush_unity_onboarding_overdrive_validate.log`. Production validation now also requires stronger physical-device combat tuning and rejects Levels 1-3 if the first enemy is not cleared before physical contact in the no-upgrade target-lock timeline estimate.

Follow Up:

Retest Levels 1-3 on a physical iPhone. The opener should feel plainly powerful: first enemy clear before collision, visible dense volleys, responsive health feedback, readable `POWER START` countdown, and no early progression wall.
...
Responded to the physical iPhone balance feedback that the first enemy still felt effectively unbeatable, fire rate felt slow, damage felt low, and early progression felt overly punishing. Added a bounded visible `POWER START` combat boost for Levels 1-3 that applies temporary damage and fire-rate multipliers at run start, shows a floating callout, and reuses the active boost HUD countdown. Tightened physical-device combat feel by increasing target range, projectile speed, visual burst density, opening salvo punch, post-gate power-spike punch, immediate hit confirmation, close-range urgency, and early rally/contact mercy margins while keeping the boost level-limited and data-driven.

Result:

`git diff --check` passed. Unity production validation passed with log `/tmp/armyrush_unity_onboarding_boost_validate2.log` after the validator caught and corrected an invalid mercy-threshold relationship. The validator now also guards the new onboarding boost duration/multipliers and the stronger first-session combat response ranges.

Follow Up:

Retest Levels 1-3 and the first enemy on the physical iPhone. The opener should now show `POWER START`, fire visibly faster, clear the first red group quickly, preserve the intended power fantasy, and still transition into normal upgrade-driven difficulty after onboarding.
...
Added data-driven multi-muzzle flash feedback to player volleys. Aggregate crowd shots now spawn a small capped spread of pooled muzzle flashes around the aim origin instead of a single flash, making small-crowd and boosted volleys read more like several soldiers firing on physical phone screens while preserving the existing army-scaled damage, cadence, projectile, immediate-hit, and power-spike balance math.

Result:

Physical-device combat should feel denser and more active without a hidden damage buff. `GlobalTuning` and the tuning asset now own `muzzleFlashVisualBurst`, the project builder regenerates it, and production validation guards the muzzle-flash burst range against invisible or noisy values.

Follow Up:

Retest Levels 1-5 on physical iPhone and verify the extra muzzle flashes improve perceived fire rate without cluttering enemies, gates, or world labels in portrait.
...
First-Enemy Timeline Validation
...
Replaced the optimistic Levels 1-3 first-enemy survivability estimate with a timeline validator that follows the actual onboarding timing more closely. The validator now starts when the first enemy enters target range, simulates best-gate timing before contact, applies opening salvos, queued post-gate power-spike volleys, immediate hit-confirmation damage, steady-fire cadence, and projectile travel delay, then checks survivors against the first-session safety target. This specifically protects the physical-device case where combat can lock onto the first enemy before the first gate is collected.

Result:

Future early-combat tuning regressions should be caught with stronger evidence instead of a best-route estimate that assumes the full grown army fires for the whole target window. `git diff --check` passed, and Unity production validation passed with log `/tmp/armyrush_unity_first_enemy_timeline_validate.log`.

Follow Up:

Retest Levels 1-3 on physical iPhone and compare perceived first-enemy pressure against the timeline validator, especially when choosing the highlighted first gate and when intentionally taking the weaker opening gate.
...
Added persistent gameplay HUD feedback for temporary run boost gates. `RunManager` now tracks active damage, fire-rate, coin, and combined boost states and updates a compact safe-area boost indicator with a countdown label and fill. `GameplayUI` owns the noninteractive indicator, creates a runtime fallback for older scenes, disables all boost graphics as raycast targets, and hides the indicator cleanly before runs and on result states. The project builder now generates serialized boost indicator references for future Game scene rebuilds and production validation checks compact placement, fill, label, boss/progress overlap, and passive raycast behavior.

Result:

Temporary `FIRE`, `DMG`, `COIN`, and combined `POWER` boosts no longer rely only on one-shot floating text. The player gets readable portrait-safe feedback while the boost is active, and the coin preview is refreshed when a coin boost expires so the HUD does not keep an inflated pending reward value. Static validation passed with `git diff --check` plus targeted source checks for the boost indicator, runtime fallback, and validator wiring.

Follow Up:

Physical iPhone QA should verify the boost indicator remains readable during Levels 6, 7, and 10 without covering gates, boss UI, or the crowd. Unity production validation was retried with log `/tmp/armyrush_unity_active_boost_hud_validate.log`, but AR-009 Unity Licensing Client protocol failures again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting, so iOS simulator export/launch was not retried in this pass.
...
Run Boost Gates and Post-Boss Power Ramp
...
Converted the existing damage, fire-rate, and coin boost gate operations from inert positive labels into real timed run power-ups. `GateController` now passes boost gates through the active `RunManager`, which owns bounded temporary multipliers for combat damage, combat fire rate, and coin rewards. Player combat now consumes those multipliers alongside permanent upgrades, opening salvos, and close-range urgency. Added a fire-rate boost pickup to Level 6, a damage boost pickup to Level 7, and a pre-boss fire-rate boost pickup to Level 10, with generator and production-validator guardrails for those placements.

Result:

The post-first-boss ramp now offers visible power spikes beyond raw army-count growth, making combat and boss preparation feel stronger while preserving permanent upgrade progression. Boost duration and max multiplier are data-driven in `GlobalTuning`, and the authored level assets match the project-builder regeneration path.

Follow Up:

Physical iPhone QA should verify that `FIRE+25%`, `DMG+25%`, and the Level 10 `FIRE+30%` pre-boss pickup are readable, rewarding, and not visually confused with soldier-count gates. Static validation passed with `git diff --check` and a run boost gate smoke check. Unity production validation was retried with log `/tmp/armyrush_unity_run_boost_gate_validate.log`, but AR-009 Unity Licensing Client protocol failures again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting.
...
Added a bounded early contact mercy path for first-session enemy collisions. In Levels 1-5, if the player reaches an enemy with a real surviving squad and the remaining enemy count is only slightly above the player count, `RunManager` can spend a single per-run mercy use, leave a tiny surviving squad, show a `LAST STAND` callout, and let the existing rally assist restore momentum. Hopeless contacts still defeat the player normally.

Result:

This catches a physical-device failure mode that static DPS checks miss: in-flight projectiles or trigger timing can make a nearly-cleared early enemy collide before the last damage resolves. Near-wins now become a dramatic survival beat instead of an instant onboarding wipe, while the data-driven minimum squad, enemy buffer, survivor count, level limit, and max uses keep the system bounded. Production validation now includes guardrails for those tuning values.

Follow Up:

Physical iPhone QA should verify that first-enemy and early-level collisions feel fair: close clears should survive with readable `LAST STAND`/rally feedback, while truly poor routes or depleted squads can still lose. Static validation passed with `git diff --check` and an early contact mercy smoke check. Unity production validation was retried with log `/tmp/armyrush_unity_contact_mercy_validate.log`, but AR-009 Unity Licensing Client protocol failures again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting.
...
Moved camera impact feedback from hardcoded cue amplitudes into data-driven `GlobalTuning` profiles for obstacle breaks, boss hits, boss defeats, and victory. `CameraFollowRig` now applies a globally scaled, clamped screen-plane shake with a squared fade-out instead of unbounded random sphere offsets, so repeated impact cues stay more readable in portrait on iPhone and iPad. Production validation now checks the camera shake profile ranges and ordering.

Result:

Camera impact polish is safer and easier to tune without code changes. Obstacle shakes remain tiny, boss/victory impacts retain a punch, and all profiles are capped by `cameraShakeMaxAmplitude` before being applied. Static validation passed with `git diff --check` and a camera-shake tuning smoke check.

Follow Up:

Physical iPhone QA should verify obstacle destruction, boss hit, boss defeat, and victory shakes feel satisfying without distracting from gates or combat. Unity production validation was retried with log `/tmp/armyrush_unity_camera_shake_validate.log`, but AR-009 Unity Licensing Client protocol failures again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting.
...
Added a data-driven early rally assist for first-session survival feel. During Levels 1-5, if the player is still alive but drops below the configured low-count threshold, `RunManager` can restore the crowd to a modest target count with normal crowd gain feedback, a `RALLY +N` callout, and light haptics. The assist is bounded by per-run use count and cooldown so it protects onboarding momentum without making defeat impossible.

Result:

Low-count survivors in early levels now get a small recovery beat before the run collapses, complementing early defeat consolation and recommended upgrade guidance. `GlobalTuning` and the project builder now serialize the rally level limit, minimum threshold, target count, max uses, and cooldown. Production validation includes guardrails for those values. Static validation passed with `git diff --check` and an early rally tuning smoke check. Unity production validation was attempted with log `/tmp/armyrush_unity_rally_assist_validate.log`, but AR-009 Unity Licensing Client protocol failures again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting.

Follow Up:

Physical iPhone QA should verify the assist triggers only after genuinely low-count mistakes, the `RALLY` callout remains readable in portrait, and early runs still allow real defeats when the army is wiped. Re-run Unity production validation once local Unity licensing is healthy.
...
Levels 2-3 Onboarding Alignment
...
Extended the direct authored onboarding approach through the first three levels. Level 2 now follows `GAMEPLAY_LOOP.md` with `+10` vs `+15`, a 20-count enemy, `x2` vs `+20`, and a 100 HP crate obstacle. Level 3 now introduces the first negative gate with `+20` vs `-10`, then a 25-count enemy, `x2` vs `+30`, and a 150 HP barricade. The lower-priority `LEVEL_DESIGN.md` labels for Level 2/3 intros were treated as a hierarchy conflict and resolved in favor of `GAMEPLAY_LOOP.md` because it is the primary gameplay source of truth.

Result:

The project builder now regenerates Levels 1-3 as direct onboarding levels while keeping reusable chunks for later authored levels and endless generation. Production validation now guards the exact Level 2 and Level 3 teaching beats. Static onboarding checks passed: Levels 1-3 clear their first enemy before contact with no upgrades and respect the existing decision-cadence limit after the crate/barricade spacing adjustment. Unity production validation was retried with log `/tmp/armyrush_unity_level23_validate.log`, but the local Unity Licensing Client issue tracked as AR-009 again prevented `ArmyRushProjectBuilder.ValidateProductionFoundation` from starting.

Follow Up:

Retest Levels 1-3 on the physical iPhone as a continuous first-session ramp, especially the Level 3 negative gate readability and the perceived difficulty after the first upgrade purchase.
...
Levels 11-15 Heavier Combat and Physical Combat Feel
...
Assets/_Project/Editor/ArmyRushProjectBuilder.cs
Assets/_Project/ScriptableObjects/Tuning/SO_GlobalTuning.asset
Assets/_Project/ScriptableObjects/Levels/SO_Level_011.asset
Assets/_Project/ScriptableObjects/Levels/SO_Level_012.asset
Assets/_Project/ScriptableObjects/Levels/SO_Level_013.asset
Assets/_Project/ScriptableObjects/Levels/SO_Level_014.asset
Assets/_Project/ScriptableObjects/Levels/SO_Level_015.asset
Docs/TASKS.md
Docs/DEVLOG.md
...
Tightened physical-device combat feel after the no-upgrade feedback by moving baseline volleys to 0.30s, extending targeting to 24m, raising projectile speed to 36, increasing the projectile visual burst cap to 6, and making small-crowd volleys visibly denser while preserving the army-count DPS model. Converted Levels 11-14 into direct authored heavier-combat layouts with larger enemy groups, x3/risk gates, and military obstacle variants. Converted Level 15 into a direct mech boss milestone with ShockwaveSlam validation, truck/turret/fuel pressure, and 10500 HP boss tuning.

Result:

`git diff --check` passed. Static authored-level validation passed for Levels 11-15: all five levels use direct data with empty chunk lists, Levels 11-14 have no boss, Level 15 references the mech boss with 10500 HP, heavier-combat best-gate counts resolve to 250, 280, 300, 300, and 300, and all obstacle collision penalties stay at or below 20. A no-upgrade balance estimate for Levels 1-15 passed; Levels 11-15 end with conservative final counts of 226, 251, 282, 264, and 246, and the Level 15 boss engagement lasts about 4.27 seconds after the pre-boss route. Unity production validation was retried with log `/tmp/armyrush_unity_level15_validate.log`, but the local Unity Licensing Client issue tracked as AR-009 again failed with unsupported protocol version `1.18.1` before `ArmyRushProjectBuilder.ValidateProductionFoundation` could execute.

Follow Up:

Retest Levels 1-15 on the physical iPhone. The run should now start cleanly, shoot sooner and more often, keep projectile feedback readable, and let the Level 15 mech boss feel like a heavier milestone without blocking no-upgrade first-session progress. Re-run Unity production validation once local Unity licensing is healthy.
...
Unity batch validation has had intermittent workstation licensing protocol failures in recent passes, but the latest `ArmyRushProjectBuilder.ValidateProductionFoundation` run completed successfully after the enemy count damage feedback pass. Latest successful validation log: `/tmp/armyrush_unity_enemy_count_pulse_validate.log`. Latest iOS simulator export/build/install/launch smoke passed after the onboarding overdrive tuning pass, with screenshot `/tmp/armyrush_onboarding_overdrive_smoke.png` and no targeted runtime exception/null-reference/missing-reference/fatal signatures. The simulator smoke still does not replace physical iPhone retesting for the original device-only touch, portrait scale, and combat-feel findings.

...
Added a dedicated pooled red enemy-defeat burst cue for cleared enemy groups. `EnemyGroup` now spawns the cue at the group when it clears, and `VfxManager` owns both a runtime fallback prefab for existing scenes and a builder-wired `PF_VFX_EnemyDefeat` path for future regenerated scenes. The effect layers with the existing count-drop pulse, `CLEAR` text, enemy-defeat audio, and coin reward feedback without changing combat balance.

Result:

Physical-device enemy clears should read more decisively, especially during the first enemy and early onboarding groups where the player needs clear proof that damage is working. The effect uses the existing pool path and prewarm flow, preserving the project-wide mobile performance requirement. `git diff --check` passed, and Unity production validation passed with log `/tmp/armyrush_unity_enemy_defeat_vfx_validate.log`; a targeted scan found no compile errors, Unity exceptions, null references, missing references, fatal signatures, or validation failures.

Follow Up:

Run physical iPhone QA to verify the red burst is readable without cluttering enemy count labels, gate text, or reward coins. Replace the procedural fallback styling with final authored VFX art during the final VFX pass.
